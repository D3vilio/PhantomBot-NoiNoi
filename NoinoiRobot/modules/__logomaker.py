...
from NoinoiRobot import OWNER_ID, BOT_USERNAME, SUPPORT_CHAT
from NoinoiRobot.events import register
from NoinoiRobot import telethn
...
async def lego(event):
 ...
 if event.sender_id != OWNER_ID and not quew:
  ...
  return
 pesan = await event.reply('`Creating your logo waimt...`')
 try:
    ...
    fillcolor = "black"
    shadowcolor = "blue"
    fnt = glob.glob("./NoinoiRobot/logopom/*")
    ...
    fname = "luna.png"
    img.save(fname, "png")
    await telethn.send_file(event.chat_id, file=fname, caption = f"Made by [Phantom ✨](https://t.me/PhantomRo_Bot)")         
    await pesan.delete()
    if os.path.exists(fname):
            os.remove(fname)
 except Exception as e:
    await event.reply(f'Error, Report **@SEVEN_7_RING**')
